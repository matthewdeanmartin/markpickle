...
1. **Format** — paste markdown → reformat with mdformat (if installed) or internal formatter
1. **Validate** — paste markdown → report on what will/won't round-trip
1. **Config** — view/edit current Config options, see their effect live
1. **Doctor** — same output as `markpickle doctor` CLI command
1. **Help / Cheat Sheet** — embedded syntax reference
