        ...
        if token["type"] == "list_item":
            for child in token["children"]:
                if child["type"] in ("block_text", "paragraph"):
                    ...
                    elif block_text["children"][0]["type"] in ("strong", "emphasis", "codespan"):
                        # Bold/italic/code-wrapped list items — strip formatting
                        current_value = strip_formatting(block_text, " ")
                        scalar = extract_scalar(current_value, config)
                        current_list.append(scalar)
                    else:
...
def process_list_of_tokens(list_of_tokens: MistuneTokenList, config: Config) -> SerializableTypes | None:
    ...
    for token in list_of_tokens:
        ...
        if token["type"] == "list":
            ...
            token["type"] == "paragraph"
            and token.get("children")
            and all(_.get("type") in ("text", "codespan", "strong", "emphasis", "linebreak") for _ in token["children"])
        ):
            # E.g. "Cat *and* Dog", which is 3 child tokens because of formatting
            # Also handles hard linebreaks (two trailing spaces → linebreak token)
